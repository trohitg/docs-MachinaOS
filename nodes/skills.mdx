...
This ensures that user customizations are always preserved in the database while the original SKILL.md files remain as defaults.
...
Skill nodes include a built-in markdown editor for viewing and editing skill instructions:
...
Skills are organized in subfolders under `server/skills/`. The skill loader uses **recursive scanning** (`rglob("SKILL.md")`) to discover skills at any nesting depth:
...
Each top-level folder appears as an option in the **Master Skill** node's folder dropdown.

### Scanning Skill Folders

The Master Skill editor includes a **folder scanner** that discovers all skills within a selected folder. When you select a folder or enter a custom path:

1. The backend recursively scans for all `SKILL.md` files in that directory
2. Returns each skill's name, description, and metadata from the YAML frontmatter
3. Skills appear in the Master Skill's left panel for enabling/disabling

You can also type a custom folder path to scan skills from any directory on disk.
