...
Pre-configured agents for specific domains. Thirteen route to `handle_chat_agent` (shared LangGraph loop). `rlm_agent` and `claude_code_agent` have dedicated handlers.
...
| Claude Code Agent | Claude Code SDK integration | Purple |
...
Delegation is fire-and-forget: the child spawns as `asyncio.create_task()`, the parent continues working, and the child broadcasts status independently. Results can be retrieved via the auto-injected `check_delegated_tasks` tool or consumed by `taskTrigger` nodes.
...
| Skill | Bottom (diamond) | Connect Master Skill |
| Tools | Bottom (diamond) | Connect Tool nodes or dual-purpose nodes |
| Task | Bottom-left (diamond) | Connect taskTrigger for delegation feedback |

### Parameters

<ParamField path="provider" type="select" required>
  AI provider: openai, anthropic, gemini, openrouter, groq, cerebras, xai, deepseek, kimi, mistral
</ParamField>
...
<CardGroup cols={2}>
  <Card title="AI Models" icon="brain" href="/nodes/ai-models">
    10 AI providers with thinking modes
  </Card>
